...
function viewCoinDetail(coinId) {
    ...
    // Switch to predictions page and highlight/filter coin
    const navItem = document.querySelector('[data-target="page-predictions"]');
    navItem.click();
    
    // In our simplified logic, we just load all but we could filter here
    setTimeout(() => {
        const card = document.querySelector(`.prediction-card[data-id="${coinId}"]`);
        if (card) {
...
async function loadPredictions() {
    ...
    try {
        ...
        grid.innerHTML = predictionData.map((coin, i) => {
            const hm = coin.hype_metrics;
            const signalClass = hm.label.toLowerCase().replace(' ', '-');
            ...
            return `
            <div class="prediction-card signal-${signalClass}" data-id="${coin.id}" style="animation-delay: ${i * 0.08}s">
                ...
                <div class="pred-header">
                    ...
                    <span class="pred-signal-badge signal-badge-${signalClass}">${hm.label.toUpperCase()}</span>
                </div>
                ...
                <div class="pred-price-row flex-col">
                    <div class="pred-price-main" style="display:flex;align-items:center;gap:10px;">
                        ...
                        <span class="pred-change ${changeColor}">${changePrefix}${coin.change_24h.toFixed(2)}%</span>
                    </div>
                    <div class="market-metrics-card" style="margin-top:12px;background:rgba(255,255,255,0.03);padding:10px;border-radius:6px;font-family:'JetBrains Mono',monospace;font-size:0.85em;display:flex;flex-direction:column;gap:4px;">
                        <div style="display:flex;justify-content:space-between;color:var(--color-cyan);"><span>Vol (24h)</span> <span>$${(hm.social_volume_24h * 1420).toLocaleString()}</span></div>
                ...
                <div class="explain-section">
                    ...
                    <div id="explain-${coin.id}" class="explain-content" style="display: none;">
                        ...
                        <div class="insight-list">
                            ...
                            ${hm.insights.map(ins => `<div class="insight-item">${ins}</div>`).join('')}
                        </div>
                    </div>
                </div>
                ...
                <div class="pred-footer" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:15px;margin-top:20px;padding-top:15px;border-top:1px solid rgba(255,255,255,0.05);">
                    <div class="consistency-gauge" style="display:flex;align-items:center;gap:10px;">
                        <div class="gauge-circle" style="width:40px;height:40px;border-radius:50%;background:conic-gradient(var(--color-${hm.confidence > 70 ? 'cyan' : (hm.confidence > 40 ? 'orange' : 'red')}) ${hm.confidence}%, rgba(255,255,255,0.1) 0);display:flex;align-items:center;justify-content:center;">
                            <div class="gauge-inner" style="width:32px;height:32px;border-radius:50%;background:var(--color-bg);display:flex;align-items:center;justify-content:center;font-size:0.7em;font-weight:700;">${hm.confidence}%</div>
                        </div>
                    ...
                    <div class="pred-footer-item">
                        ...
                        <span class="pred-footer-value">${hm.social_volume_24h.toLocaleString()}</span>
                    </div>
                    <div class="pred-footer-item">
                        <span class="pred-footer-label">7D CHANGE</span>
                        <span class="pred-footer-value ${coin.change_7d >= 0 ? 'text-green' : 'text-red'}">${coin.change_7d > 0 ? '+' : ''}${coin.change_7d.toFixed(1)}%</span>
                    </div>
                    <div class="pred-footer-item">
